...
function ProcessTable() {
  ...
  const handleDeleteConfirm = async () => {
    try {
      await axios.delete(`http://localhost:4000/api/lots/${selectedEntry.subEntry._id}`);
      fetchSubmittedData();
                  ...
                  <React.Fragment key={index}>
                    {entry.entries.map((subEntry, subIndex) => (
                      <tr key={subEntry._id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">{formatDateTime(subEntry.submittedAt) || "N/A"}</td>
